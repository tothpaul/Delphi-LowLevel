...
{
   Cross-platform OpenGL implementation for Delphi XE2/XE5/Berlin/Tokyo
...
// base class for a single form OpenGL application
  ...
  {$IFDEF Android}
  private
    FApp          : TAndroidApplication;
    FDisplay      : EGLDisplay;
    FSurface      : EGLSurface;
    FContext      : EGLContext;
    FConfigChanged: Boolean;
    procedure OnAppCmd(Cmd: ShortInt);
  {$ENDIF}
...
{$IFDEF Android}
procedure TGLContext.onAppCmd(Cmd: ShortInt);
begin
  case cmd of
    APP_CMD_INIT_WINDOW:
    begin
